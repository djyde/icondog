...
export function RemoteIconSetPage() {
  const iconSetsQuery = useQuery({
    ...
    queryFn: async () => {
      const collections = await axios.get<Record<string, IconifyInfo>>(
        `https://unpkg.com/@iconify/json@${ICONIFY_VERSION}/collections.json`
      )

      return Object.entries(collections.data)
        .map(([prefix, info]) => {
          return {
            prefix,
            ...info
        ...
        <div className="">
          <div className="grid grid-cols-3 gap-5">
            {collections?.map((collection) => {
              return <IconSetCard key={collection.prefix} iconifyInfo={collection} />
            })}
          </div>
        </div>
      </div>
...
function IconSetCard(props: {
  ...
  const deleteIconSetMutation = useMutation({
    mutationFn: async (prefix: string) => {
      await window.api.deleteIconSet(prefix)
      localIconSetQuery.refetch()
    ...
    <div className="border rounded-lg">
        ...
        <div className="flex justify-start gap-3 px-3 pt-3 rounded-t-lg">
          {collection.samples?.map((label) => {
            return (
              <img
                className="w-8"
                src={`https://api.iconify.design/${collection.prefix}/${label}.svg`}
                key={label}
                alt=""
              />
            )
      ...
      <div className="p-3 space-y-1">
        ...
        <div className="text-xs text-default-400 space-y-1">
          ...
          <div>{collection.license.title}</div>
        </div>
        <div className="flex justify-end gap-3">
          {hasInstalled && (
            <Button
              ...
              color="default"
              variant="flat"
              onClick={() => deleteIconSetMutation.mutate(collection.prefix)}
            >
              Delete
            </Button>
          )}
          {!hasInstalled && (
            <Button
              ...
              size="sm"
              isLoading={downloadIconSetMutation.isPending}
              color="primary"
              variant="flat"
              onClick={() => {
